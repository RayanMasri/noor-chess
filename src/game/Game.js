								...
								<div className='inner'>
									...
									<div className='name'>
										...
										{Object.keys(state.elos).length == 2 && (
											<div style={{ marginLeft: 5, color: 'gray' }}>{Object.entries(state.elos).filter((item) => item[0] != sessionStorage.getItem('identity'))[0][1]}</div>
										)}
									</div>
									<Divider style={{ width: '100%', backgroundColor: 'white' }} />
									<div className='name'>
										<div>{state.name}</div>
										{Object.keys(state.elos).length == 2 && <div style={{ marginLeft: 5, color: 'gray' }}>{state.elos[sessionStorage.getItem('identity')]}</div>}
									</div>
									{state.timeInfo.duration > 0 ? (
										<div
